...
import { Container, Card, CardContent, Typography, CardActions, Button, useMediaQuery, Box } from '@mui/material'
...
export const HomeBanner = (props: {imageAlt: string, cardHeading: string, cardButtonLabel: string}) => {
  const {imageAlt, cardHeading, cardButtonLabel} = props
  const isTabletOrMobile = useMediaQuery('(max-width:1000px)')
  if(isTabletOrMobile){
    return (
      <Container maxWidth="sm" sx={{marginBottom: 10, boxShadow: '10px 10px 20px black' }} style={{ display: "grid", borderRadius: 10, padding: 0, gridTemplateRows: '0.6fr 0.5fr' }}>
                <StaticImage
                  layout="fullWidth"
                  alt={imageAlt}
                ...
                >
                  ...
                  <Button sx={{marginTop: 5, marginBottom: 5}} size="large" variant="contained" color="primary">{cardButtonLabel}</Button>
                  </Container>
              </Container>
    )
  }
